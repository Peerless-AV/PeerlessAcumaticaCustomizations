...
- Package **Shipping Instruction 12** contains a function critical for the new commercial invoice functionality. Once deployed, that project/report can be updated, tested, and confirmed working.
- Interestar added functionality into their base packages, negating the need for the **Interastar SEARS** package and the **Factora** package.
...
1. Upload new packages and confirm Levels.  
2. Highlight and replace any packages that require it (Interestar packages typically require highlight-and-replace).  
3. Ensure 1st-pass check marks are correct. Uncheck all MX packages. All remaining packages should be ready for deployment to all tenants. This is also a good time to delete any old packages.  
4. Deploy to all tenants.  
5. Check CloudInfo and Int.CEMT for deployment and deploy to MX tenants.  
6. Check InterestarDD and deploy to all MX tenants.  
